...
export interface Theme { accent: string; background: string; text: string; cardColor?: string; headingColor?: string; descColor?: string; }
...
export interface MenuData { ok: boolean; restaurant: Restaurant; items: MenuItem[]; categories: Category[]; platformUrl?: string; subscription_expired?: boolean; }
...
function ShareButton({ accent, slug, platformUrl }: { accent: string; slug: string; platformUrl?: string }) {
    ...
    return (
        <button onClick={handleShare}
            aria-label="Share"
            style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "40px", height: "40px", borderRadius: "12px", border: `1px solid ${accent}30`, background: `${accent}12`, color: accent, cursor: "pointer", flexShrink: 0, transition: "background 0.15s" }}
                        ...
                        <div style={{ display: "flex", overflowX: "auto", scrollSnapType: "x mandatory", WebkitOverflowScrolling: "touch" }}>
                            {item.images.map((img, i) => (
                                <div key={i} style={{ width: "100%", height: "320px", flexShrink: 0, background: "#F3F3F3", overflow: "hidden", scrollSnapAlign: "start" }}>
                                    {/* eslint-disable-next-line @next/next/no-img-element */}
                                    <img src={img.startsWith("http") ? img : `${API}${img}`} alt={`${item.name} - ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", opacity: item.available ? 1 : 0.5 }} />
                                </div>
                            ))}
                        </div>
...
// ─── Interactive Menu (CSR) ───────────────────────────────────────────────────
/**
 * All interactive menu logic lives here as a client component.
 * It receives pre-fetched server data as props, so hydration is instant
 * and no client-side API call is needed on first render.
 */
export default function MenuClient({ data, slug }: { data: MenuData; slug: string }) {
    ...
    // Track menu visit once on mount
    useEffect(() => {
        ...
        <div style={{ minHeight: "100vh", background: bg, fontFamily: "'Inter', system-ui, -apple-system, sans-serif", WebkitFontSmoothing: "antialiased", paddingBottom: "80px" }}>
            ...
            <footer style={{ maxWidth: "720px", margin: "0 auto", padding: "0 16px 48px" }}>
                ...
                <p style={{ fontSize: "11px", color: "#CACACA", textAlign: "center", marginTop: 4 }}>
                    Powered by{" "}
                    <a href={`https://spryon.app?utm_source=bio_menu&utm_medium=footer&utm_campaign=${encodeURIComponent(data.restaurant.name ?? "restaurant")}`}
